...
include('conexion.php');
    ...
    $name = $_GET['name'];
    $type = $_GET['type'];
    $idSpotify = $_GET['idSpotify'];
    $sql = "INSERT INTO historial (nombre, tipo, horario, usuarioId, idSpotify) VALUES ('$name','$type','$tiempo','$usuario', '$idSpotify')";
    ...
    // print($a['COUNT(*)']);
    if ($a['COUNT(*)'] == 0) {
        $sql = "INSERT INTO biblioteca (tipo, nombre, usuarios_id, repeticion, idSpotify) VALUES ('$type','$name','$usuario',1, '$idSpotify')";
        $artistas = mysqli_query($conexion, $sql);
        // echo 'ayuda';
    }
    while ($canta = mysqli_fetch_assoc($library)){
        if($canta["nombre"]== $name && $canta["tipo"] == $type && $canta["usuarios_id"] == $usuario) {
            ...
            $sql = "UPDATE biblioteca SET repeticion = '$repe' WHERE tipo = '$type' AND nombre = '$name' AND usuarios_id = '$usuario' AND idSpotify = '$idSpotify'";
            $artistas = mysqli_query($conexion, $sql);
        }
        // $i += 1;
    }
